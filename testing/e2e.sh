...
function test_new_container_push_only_push_changed_unchanged() {
  # test that if the digest hasnt changed and only_push_changed is True that only one tag is published
  cd "${ROOT}"
  clear_docker_full
  cid=$(docker run --rm -d -p 5000:5000 --name registry registry:2)
  ...
  EXPECT_CONTAINS "$(bazel run @io_bazel_rules_docker//tests/container:new_push_test_only_push_changed_unchanged_tag_2 2>&1)" "Successfully pushed docker image"
  EXPECT_CONTAINS "$(curl localhost:5000/v2/docker/test/tags/list)" '{"name":"docker/test","tags":["unchanged_tag1"]}'
}

function test_new_container_push_only_push_changed_changed() {
  # test that if the digest changes and only_push_changed is True that two tags are published
  cd "${ROOT}"
  clear_docker_full
  cid=$(docker run --rm -d -p 5000:5000 --name registry registry:2)
  EXPECT_CONTAINS "$(bazel run @io_bazel_rules_docker//tests/container:new_push_test_only_push_changed_changed_tag_1 2>&1)" "Successfully pushed docker image"
  EXPECT_CONTAINS "$(bazel run @io_bazel_rules_docker//tests/container:new_push_test_only_push_changed_changed_tag_2 2>&1)" "Successfully pushed docker image"
...
test_new_container_push_only_push_changed_unchanged
test_new_container_push_only_push_changed_changed
