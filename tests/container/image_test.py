...
class ImageTest(unittest.TestCase):
    ...
    def assertDigestMatchesGolden(self, img, golden_id_suffix=None):
        self.goldenTest(img.digest(), golden_id_suffix)

    def goldenTest(self, contents, golden_id_suffix=None):
        golden_id = self.id().removeprefix('__main__.')
        if golden_id_suffix:
            golden_id = f'{golden_id}-{golden_id_suffix}'
        write_goldens_to = os.environ.get('WRITE_GOLDENS_TO')
        if write_goldens_to:
            with open(os.path.join(write_goldens_to, golden_id), 'w') as f:
                print(f'writing golden copy to {f.name}')
                f.write(contents)
            return
        filename = os.path.join(TEST_DATA_TARGET_BASE, 'goldens', golden_id)
        helpmsg = f'''from golden file "{filename}"
To regenerate, rerun with --test_env WRITE_GOLDENS_TO=$PWD/{TEST_DATA_TARGET_BASE}/goldens'''
        try:
            with open(TestRunfilePath(filename)) as f:
                self.assertEqual(
                        contents, f.read(),
                        msg=helpmsg)
    ...
    def test_files_base(self):
        with TestImage('files_base') as img:
            self.assertDigestMatchesGolden(img)
    ...
    def test_data_path(self):
        ...
        with TestImage('no_data_path_image') as img:
            self.assertDigestMatchesGolden(img, 'without')
            self.assertEqual(1, len(img.fs_layers()))
            self.assertTopLayerContains(img, ['.', './test'])
        with TestImage('data_path_image') as img:
            self.assertDigestMatchesGolden(img, 'with')
            self.assertEqual(1, len(img.fs_layers()))
        ...
        with TestImage('absolute_data_path_image') as img:
            self.assertDigestMatchesGolden(img, 'absolute')
            self.assertEqual(1, len(img.fs_layers()))
            self.assertTopLayerContains(img, [
        ...
        with TestImage('root_data_path_image') as img:
            self.assertDigestMatchesGolden(img, 'root')
            self.assertEqual(1, len(img.fs_layers()))
            self.assertTopLayerContains(img, [
                '.', './testdata', './testdata/test', './testdata/test/test'])
